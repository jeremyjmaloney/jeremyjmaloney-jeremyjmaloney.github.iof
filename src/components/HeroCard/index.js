export const HeroCard = ({isRow = false, image, primaryText = "", secondaryText="", hasCta = false, ctaText="", href="#", id="", bgColor= "", margin="0", padding="0" }) => {
  return(
    <div id={id} className={`${bgColor} flex ${isRow ? "flex-row" : "flex-col"} items-center h-fit ${margin} ${padding}`}>
      <img src={image} className="rounded-2xl w-60 grayscale hover:blur-sm" alt='profile headshot' />
      {primaryText && (
