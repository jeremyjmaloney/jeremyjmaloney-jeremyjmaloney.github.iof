export const HeroCard = ({isRow = false, image, imageWidth="w-40", primaryText = "", secondaryText="", hasCta = false, ctaText="", href="#", id="", bgColor= "", margin="0", padding="0" }) => {
  return(
    <div id={id} className={`${bgColor} flex ${isRow ? "flex-row" : "flex-col"} items-center h-fit ${margin} ${padding}`}>
      <img src={image} className={`rounded-2xl ${imageWidth} grayscale hover:-skew-y-6`} alt='profile headshot' />
      {primaryText && (
      ...
      {hasCta && (
        <a href={href} className="rounded-md bg-cyan-400 px-3.5 py-2.5 text-sm font-semibold text-stone-600 shadow-xs hover:bg-yellow-300 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-yellow-300 duration-200 ease-in-out hover:-translate-y-1 hover:scale-110">
        {ctaText}
      </a>
      )}
    </div>
